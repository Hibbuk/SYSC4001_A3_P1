...
const unsigned int QUANTUM = 100; // Time quantum in milliseconds
...
std::tuple<std::string /* add std::string for bonus mark */ > run_simulation(std::vector<PCB> list_processes) {
    ...
    unsigned int time_slice_remaining = QUANTUM; // Track remaining quantum time
    PCB running;
    ...
    while(job_list.empty() || !all_process_terminated(job_list)){
        ...
        //Inside this loop, there are three things 
        ...
        if(running.state == RUNNING) {
            ...
            if(running.remaining_time == 0) {
                ...
                idle_CPU(running);
                time_slice_remaining = QUANTUM; // Reset quantum
            }
            ...
            // Check if quantum expired (PREEMPTION for Round Robin)
            else if(time_slice_remaining == 0) {
                //Time quantum expired - preempt the process
                running.state = READY;
                ready_queue.insert(ready_queue.begin(), running); // Add to front (FIFO)
                sync_queue(job_list, running);
        ...
        if(running.state == NOT_ASSIGNED && !ready_queue.empty()) {
            ...
            // Round Robin uses FIFO - no sorting, just pick from back
            ...
            time_slice_remaining = QUANTUM; // Reset quantum for new process
        }
...
int main(int argc, char** argv) {
    ...
    // Create output path in RR/Outputs folder
    std::string output_path = "RR/Outputs/execution_" + base_name + ".txt";
