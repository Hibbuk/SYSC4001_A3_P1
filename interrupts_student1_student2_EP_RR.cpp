...
std::tuple<std::string /* add std::string for bonus mark */ > run_simulation(std::vector<PCB> list_processes) {
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) {
        ...
        if(running.state == RUNNING) {
            ...
            // Check if quantum expired (PREEMPTION for Round Robin with Priorities)
            else if(time_slice_remaining == 0) {
                //Time quantum expired - preempt the process
                running.state = READY;
                ready_queue.insert(ready_queue.begin(), running); // Add to front
                sync_queue(job_list, running);
...
int main(int argc, char** argv) {
    ...
    // Create output path in EP_RR/Outputs folder
    std::string output_path = "EP_RR/Outputs/execution_" + base_name + ".txt";
